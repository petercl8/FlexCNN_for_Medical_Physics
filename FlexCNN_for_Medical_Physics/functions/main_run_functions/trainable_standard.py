...
def run_trainable(config, paths, settings):
    ...
    # DEBUG: Log augmentation received by trainable
    print(f"[TRAINABLE DEBUG] network_type={network_type}, run_mode={run_mode}")
    print(f"[TRAINABLE DEBUG] augment={augment}")
    ...
    if run_mode == 'train':
        start_lr = gen_opt.param_groups[0]['lr']
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    ...
    if network_type in ('ACT', 'CONCAT', 'GAN'):
