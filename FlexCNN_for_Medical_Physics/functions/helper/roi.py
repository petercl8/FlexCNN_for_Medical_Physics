...
def ROI_NEMA_hot(ground_truth_tensor, reconstruction_tensor, hot_background_mask, hot_mask):
    # Convert all inputs to numpy
    hot_background_mask = _to_numpy(hot_background_mask)
    ...
    background_truth = np.sum(np.multiply(hot_background_mask, ground_truth_tensor))
    background_reconstruction = np.sum(np.multiply(hot_background_mask, reconstruction_tensor))
...
def ROI_NEMA_cold(reconstruction_tensor, cold_background_mask, cold_mask):
    # Convert all inputs to numpy
    cold_background_mask = _to_numpy(cold_background_mask)
    reconstruction_tensor = _to_numpy(reconstruction_tensor)
    cold_mask = _to_numpy(cold_mask)

    background_reconstruction = np.sum(np.multiply(cold_background_mask, reconstruction_tensor))
