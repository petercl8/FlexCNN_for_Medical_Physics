"""
Stage 1 Comprehensive Test Suite: Data Loading with Optional Attenuation Maps

Tests:
1. Backward compatibility: Normal mode (atten_map_path=None) returns proper nested tuples
2. Attenuation map loading: Shapes and dtypes match activity maps
3. Augmentation consistency: Same transforms applied to attenuation and activity data
4. Error handling: Missing files raise appropriate errors
5. Nested tuple unpacking: Works correctly in run_supervisory.py context
"""
...
from FlexCNN_for_Medical_Physics.classes.dataset_classes import NpArrayDataSet, NpArrayDataLoader
...
def create_dummy_arrays(num_examples=10, sino_shape=(3, 288, 288), image_shape=(1, 180, 180)):
    """Create dummy numpy arrays for testing"""
    sino_array = np.random.randn(num_examples, *sino_shape).astype(np.float32)
    image_array = np.random.randn(num_examples, *image_shape).astype(np.float32)
    atten_array = np.random.randn(num_examples, *image_shape).astype(np.float32)
    recon1_array = np.random.randn(num_examples, *image_shape).astype(np.float32)
    recon2_array = np.random.randn(num_examples, *image_shape).astype(np.float32)
    return sino_array, image_array, atten_array, recon1_array, recon2_array
...
def test_backward_compatibility_no_augmentation():
    ...
    try:
        ...
        # Create dummy arrays
        sino_array, image_array, atten_array, recon1_array, recon2_array = create_dummy_arrays()
        
        # Save to temp files
        sino_path = os.path.join(tmpdir, 'sino.npy')
        image_path = os.path.join(tmpdir, 'image.npy')
        recon1_path = os.path.join(tmpdir, 'recon1.npy')
        recon2_path = os.path.join(tmpdir, 'recon2.npy')
        ...
        # Create dataset WITHOUT attenuation maps
        ...
        # Close dataset explicitly and collect garbage
        if dataset is not None:
            dataset.close()
            del dataset
        gc.collect()
        # Now delete temp directory
...
def test_attenuation_map_loading():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sino_path = os.path.join(tmpdir, 'sino.npy')
            image_path = os.path.join(tmpdir, 'image.npy')
            atten_path = os.path.join(tmpdir, 'atten.npy')
            ...
            # Close dataset before exiting tempdir context
            if dataset is not None:
                dataset.close()
                del dataset
    finally:
...
def test_augmentation_consistency():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create dummy arrays with distinct patterns to detect augmentation
            sino_array = np.ones((10, 3, 288, 288), dtype=np.float32)
            image_array = np.ones((10, 1, 180, 180), dtype=np.float32)
            atten_array = np.ones((10, 1, 180, 180), dtype=np.float32) * 2  # Distinct value
...
def test_error_handling_missing_attenuation_file():
    """Test that missing attenuation map file raises error"""
    print("\n" + "="*80)
    print("TEST 4: Error Handling (Missing Attenuation File)")
    print("="*80)
    
    with tempfile.TemporaryDirectory() as tmpdir:
        # Create only sino and image, not attenuation
        sino_array, image_array, _, _, _ = create_dummy_arrays()
        
        sino_path = os.path.join(tmpdir, 'sino.npy')
        image_path = os.path.join(tmpdir, 'image.npy')
        atten_path = os.path.join(tmpdir, 'atten_MISSING.npy')  # This file doesn't exist
        ...
        # Attempt to create dataset with non-existent attenuation file
        try:
            dataset = NpArrayDataSet(
                image_path=image_path,
                sino_path=sino_path,
                ...
                recon1_path=None,
                recon2_path=None,
                atten_map_path=atten_path  # Non-existent file
            )
            # Try to access data
            dataset[0]
            print("❌ FAIL: Should have raised an error for missing attenuation file")
            assert False, "Should raise error for missing file"
        except (FileNotFoundError, OSError, ValueError) as e:
            print(f"✅ PASS: Correctly raised error for missing attenuation file")
            print(f"   - Error type: {type(e).__name__}")


# ========================================================================================
# TEST 5: NESTED TUPLE UNPACKING IN run_supervisory.py CONTEXT
# ========================================================================================

def test_nested_tuple_unpacking():
    """Test that nested tuples unpack correctly in a run_supervisory.py-like context"""
    print("\n" + "="*80)
    print("TEST 5: Nested Tuple Unpacking (run_supervisory.py Context)")
    print("="*80)
...
if __name__ == '__main__':
    print("\n" + "="*80)
    print("STAGE 1: COMPREHENSIVE DATA LOADING TEST SUITE")
    print("="*80)
    ...
    tests = [
        ...
        test_error_handling_missing_attenuation_file,
