...
class Generator_288(nn.Module):
    def __init__(self, config, gen_SI=True, gen_skip_handling: str = '1x1Conv', gen_flow_mode: str = 'coflow', frozen_enc_channels=None, frozen_dec_channels=None, enable_encoder_mixer=True, enable_decoder_mixer=True, scaling_exp=0.7):
        ...
        Input Size Handling:
            - SI mode (gen_SI=True): Input must be exactly 288x288 (from gen_sino_size).
            - IS mode (gen_SI=False): Input size can be ≤288. If smaller than 288x288, 
              the input will automatically be zero-padded to 288x288 (symmetric/centered)
              before the encoder. Output remains full 288x288 sinogram.
        ...
        # Channel references for mixing (144, 36, 9 scales)
    ...
    def forward(self, input, frozen_encoder_features=None, frozen_decoder_features=None, return_features: bool = False):
        ...
        if self.pad_input:
            ...
            hidden = nn.functional.pad(hidden, (pad_left, pad_right, pad_top, pad_bottom))
...
class Generator_256(nn.Module):
    def __init__(self, config, gen_SI=True, gen_skip_handling: str = '1x1Conv', gen_flow_mode: str = 'coflow', frozen_enc_channels=None, frozen_dec_channels=None, enable_encoder_mixer=True, enable_decoder_mixer=True, scaling_exp=0.7):
        ...
        Input Size Handling:
            - SI mode (gen_SI=True): Input must be exactly 256x256 (from gen_sino_size).
            - IS mode (gen_SI=False): Input size can be ≤256. If smaller than 256x256, 
              the input will automatically be zero-padded to 256x256 (symmetric/centered)
              before the encoder. Output remains full 256x256 sinogram.
...
class Generator_320(nn.Module):
    def __init__(self, config, gen_SI=True, gen_skip_handling: str = '1x1Conv', gen_flow_mode: str = 'coflow', frozen_enc_channels=None, frozen_dec_channels=None, enable_encoder_mixer=True, enable_decoder_mixer=True, scaling_exp=0.7):
        ...
        Input Size Handling:
            - SI mode (gen_SI=True): Input must be exactly 320x320 (from gen_sino_size).
            - IS mode (gen_SI=False): Input size can be ≤320. If smaller than 320x320, 
              the input will automatically be zero-padded to 320x320 (symmetric/centered)
              before the encoder. Output remains full 320x320 sinogram.
...
class Generator_180(nn.Module):
    def __init__(self, config, gen_SI=True, gen_skip_handling: str = '1x1Conv', gen_flow_mode: str = 'coflow', frozen_enc_channels=None, frozen_dec_channels=None, enable_encoder_mixer=True, enable_decoder_mixer=True, scaling_exp=0.7):
        ...
        Input Size Handling:
            - SI mode (gen_SI=True): Input must be exactly 180x180 (from gen_sino_size).
            - IS mode (gen_SI=False): Input size can be ≤180. If smaller than 180x180, 
              the input will automatically be zero-padded to 180x180 (symmetric/centered)
              before the encoder. Output remains full 180x180 sinogram.
