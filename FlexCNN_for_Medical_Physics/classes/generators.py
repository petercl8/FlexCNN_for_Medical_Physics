...
class Generator_320(nn.Module):
    def __init__(self, config, gen_SI=True):
        ...
        # Conv2d formula: H_out = floor((H_in + 2*padding - kernel) / stride) + 1
        self.contract_blocks = nn.ModuleList([
            contract_block(in_chan, dim_0, 3, stride=2, padding=1, padding_mode=pad, fill=fill, norm=norm, drop=drop),   # H = (320+2-3)/2+1 = 160
            contract_block(dim_0, dim_1, 3, stride=2, padding=1, padding_mode=pad, fill=fill, norm=norm, drop=drop),     # H = (160+2-3)/2+1 = 80
            contract_block(dim_1, dim_2, 3, stride=2, padding=1, padding_mode=pad, fill=fill, norm=norm, drop=drop),     # H = (80+2-3)/2+1 = 40
            contract_block(dim_2, dim_3, 3, stride=2, padding=1, padding_mode=pad, fill=fill, norm=norm, drop=drop),     # H = (40+2-3)/2+1 = 20
    ...
    def _build_neck(self, neck, dim_3, dim_4, z_dim, pad, fill, norm, drop):
        ...
        if neck == 'narrow':
            ...
            return nn.Sequential(
                ...
                contract_block(dim_4, dim_4, 3, stride=2, padding=1, padding_mode=pad, fill=fill, norm=norm, drop=drop),      # H = (5+2-3)/2+1 = 3
                contract_block(dim_4, z_dim, 3, stride=1, padding=0, padding_mode=pad, fill=0, norm='batch', drop=False),     # H = (3+0-3)/1+1 = 1
                expand_block(z_dim, dim_4, 3, stride=2, padding=0, output_padding=0, padding_mode=pad, fill=fill, norm=norm, drop=drop),  # H = (1-1)*2+3-0+0 = 3
                expand_block(dim_4, dim_4, 4, stride=2, padding=2, output_padding=1, padding_mode=pad, fill=fill, norm=norm, drop=drop),  # H = (3-1)*2+4-4+1 = 5
